...
class InterparkCombineSpider(HiaasCommon):
    ...
    def parse(self, response):
        ...
        with sync_playwright() as pw:
            ...
            # page.goto('https://shopping.interpark.com/product/productInfo.do?prdNo=6173396273&dispNo=008001083&smid1=common_prd')

            # sleep(INTERPARK_CRAWL_DELAY)
            # title = page.locator('//*[@id="title_tmp"]/span[2]').nth(0).inner_text()
            # title2 = page.locator('//*[@id="priceWrap"]/div[2]/span[1]/em').nth(0).inner_text()
            ...
            for keyword in INTERPARK_KEYWORD_LIST:
                for pagenum in range(1, INTERPARK_PAGE_COUNT + 1):
                    ...
                    if pagenum == 1 :
                        end_ad_rank = int(locators.nth(0).get_attribute('data-srch-list-order')) - 1
                    logging.log(logging.INFO, "end_ad_rank = %d", end_ad_rank)

                    for i in range(count):
                        locator = locators.nth(i)

                        tmp = locator.locator('//*[@class="name"]').inner_text().lower().replace(' ', '')
                        print("상품명 (풀네임) : " + tmp)
                        if keyword == INTERPARK_KEYWORD_LIST[0]:
                            if ('우머나이저' in tmp) or ('womanizer' in tmp):
                                brandYn = True
                            else: brandYn = False
                        elif keyword == INTERPARK_KEYWORD_LIST[1]:
                            if ('퓨어젤' in tmp):
                                brandYn = True
                            else: brandYn = False
                        elif keyword == INTERPARK_KEYWORD_LIST[2]:
                            if ('아크웨이브' in tmp) or ('arcwave' in tmp):
                                brandYn = True
                            else: brandYn = False
                        elif keyword == INTERPARK_KEYWORD_LIST[3]:
                            if ('롬프' in tmp) or ('romp' in tmp):
                                brandYn = True
                            else: brandYn = False
                        elif keyword == INTERPARK_KEYWORD_LIST[4]:
                            if ('위바이브' in tmp) or ('wevibe' in tmp) or ('we-vibe' in tmp):
                                brandYn = True
                            else: brandYn = False
                        elif keyword == INTERPARK_KEYWORD_LIST[5]:
                        ...
                        if brandYn:
                            ...
                            tmp_rank = int(locator.get_attribute('data-srch-list-order'))
                            rank = tmp_rank - end_ad_rank
                            logging.log(logging.INFO, "rank = %d", rank)

                            href = locator.locator('//a[@class="name"]').get_attribute('href')
                            ...
                            # 판매자정보(사업자 명)
                            seller = locator.locator('//*[@class="cname"]').inner_text()
                            # print("판매자정보(사업자 명) : " + seller) 
                            # 상품번호 
                            item_no = locator.locator('//a[@class="name"]').get_attribute('data-item_no')
                            logging.log(logging.INFO, "item_no = %s", item_no)

                            # 상품명 (풀네임)
                            title = locator.locator('//*[@class="name"]').inner_text()
                            ...
                            # 판매가(쿠폰 포함)
                            price = int((locator.locator('//*[@class="number"]').inner_text()).replace(",","").strip())
                            ...
                            # 가격할인정보
                            if locator.locator('//*[@class="sale"]').inner_text() == "":
                                discount = price
                                # print("할인 없는 원가정보 : " + discount)
                            else:
                                discount = locator.locator('//*[@class="under"]').inner_text()
